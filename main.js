...
linkEmail.addEventListener('click',
  () => {
    ...
    const emailSubject =
      'Link Website';
    const emailBody = `Silakan kunjungi link berikut ini:
${currentUrlLe}`;
    const emailUrl =
...
linkMore.addEventListener('click',
  () => {
    if (navigator.canShare) {
      navigator.share({
          title: 'Bagikan Link',
          text: 'Silakan kunjungi link berikut:',
          url: currentUrlLm,
        })
        .then(() => {
          alert(
            'Link berhasil dibagikan!'
          );
          timer = setTimeout(() => {
            a.style.transition =
              'all .7s ease-in-out';
      ...
      alert(
        'Fitur bagikan tidak didukung oleh browser ini'
      );
      timer = setTimeout(() => {
        a.style.transition =
          'all .7s ease-in-out';
